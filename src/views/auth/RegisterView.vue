...
import { register, RoleId, type RoleIdType } from '../../services/auth'
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex rounded-lg bg-slate-100 p-0.5" role="tablist">
      <button
        type="button"
    ...
    <form class="flex flex-col gap-4" @submit.prevent="onSubmit">
      <div>
        ...
        <div class="relative">
          ...
          <button
            v-if="account"
            type="button"
            class="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-400 hover:bg-slate-100"
            aria-label="清除"
            :disabled="loading"
            @click="clearAccount"
          >
      ...
      <div>
        <label class="mb-1 block text-xs font-medium text-slate-600">验证码</label>
        <div class="flex gap-2">
          <input
            ...
            inputmode="numeric"
            maxlength="6"
            class="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2.5 text-slate-900 outline-none ring-sky-500 focus:border-sky-500 focus:ring-2"
            placeholder="请输入短信验证码"
            :disabled="loading"
          />
          <button
            type="button"
            class="shrink-0 rounded-lg border border-sky-600 px-3 text-sm font-medium text-sky-600 hover:bg-sky-50 disabled:border-slate-200 disabled:text-slate-400 disabled:hover:bg-transparent"
      ...
      <button
        type="submit"
        class="flex w-full items-center justify-center gap-2 rounded-lg bg-sky-600 py-3 text-sm font-medium text-white transition hover:bg-sky-700 disabled:opacity-60"
        :disabled="loading"
      >
        <svg
          v-if="loading"
